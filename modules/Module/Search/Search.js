...
sMap.Module.Search = OpenLayers.Class(sMap.Module, {
	...
	creatingwebparams : function(){
		...
		if(poiLayer == null || poiLayer.features.length != 1){
			...
			sMap.db.webParams.poi = self.lastSearchedPoi;
		}
		else{
	...
	afterapplyingwebparams : function(){
		...
		if (paramsObj.POI) {
			...
			sMap.events.triggerEvent("addmarkerataddress", this, {
			    poi : paramsObj.POI,
			    zoom : paramsObj.ZOOMLEVEL || this.zoomLevel
	...
	drawContent : function() {
		...
		if (self.dropDownOption == true) {
			...
			if (self.appendAsOwnDiv != true){
				...
				var closeBtn = $("<div id='search-btnclose' class='ui-button-icon-primary ui-icon ui-icon-close' title='" + self.lang.closeOptions + "' />");
				var header = $("<div id='search-header'>Sökalternativ</div>");
	...
	/**
	 * Will append drop-down to a div underneath the searchBox.
	 * The appendAsOwnDiv-option must be set to true in Search_conf.js.
	 *   
     */
	
	appendAsDiv : function() {
		var optionsMenuDiv = this.optionsMenuDiv,
			...
			if (setCenter === true) {
                if (feature.geometry.CLASS_NAME == "OpenLayers.Geometry.Polygon"||feature.geometry.CLASS_NAME == "OpenLayers.Geometry.LineString"){
                    ...
                    sMap.map.setCenter(new OpenLayers.LonLat(feature.geometry.x, feature.geometry.y), theZoom);
						...
						/* Confused by center and setCenter. this code might not be relevant.
						if (Center != null){
							theCenter = typeof(theCenter)=="string" ? theCenter.split(",") : theCenter;
							var east = parseInt(theCenter[0]);
							var north = parseInt(theCenter[1]);
							sMap.map.setCenter(new OpenLayers.LonLat(east, north), theZoom);
						}
						else {sMap.map.setCenter(new OpenLayers.LonLat(feature.geometry.x, feature.geometry.y), theZoom);}
						*/
		}
