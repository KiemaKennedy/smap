	...
	createChoiceDialog : function(features, options) {
		...
		if (features.length) {	
			...
			for (var i=0; i<len; i++) {
				...
				/*if (t==null) {
					var test = utils.getLayersWithProperty("getFeatureInfo.layers", name);
					t = (test && test.length) ? test[0] : null;
				}*/
				
				var iconURL = t.style && t.style["default"] && t.style["default"].externalGraphic ?
						t.style["default"].externalGraphic :
							(t.legend ? t.legend.url : null);
				
				var rowTag = $("<div id='"+rowId+"' unselectable='on' class='unselectable selectdialog-rowtag'><img src='"+iconURL+"'></img><div>"+t.displayName + "</div></div>");
				dialogDiv.append(rowTag);
				rows += 1;
