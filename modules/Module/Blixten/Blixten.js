...
sMap.Module.Blixten = OpenLayers.Class(sMap.Module, {
	...
	activate : function() {
		...
		// Hidden egg
		$("#toolbar-maindiv").unbind("dblclick").dblclick(function(e) {
			if (e.altKey) {
	...
	deactivate : function() {
		...
		$("#toolbar-maindiv").unbind("dblclick"); // remove hidden egg
...
//		this.layer.destroyFeatures();
	...
	drawContent : function() {
		...
		this.layer.events.register("sketchcomplete", this, function(e) {
			this.loading(true);
			this.drawPointControl.deactivate(); // Temporarily deactivate the drawing tool
			this.layer.destroyFeatures();
			this.doRequest(e.feature.geometry);
	...
	loading: function(change) {
		...
		if (change === true) {
			...
			sMap.events.triggerEvent("loadingdone", this, {
				text: this.lang.textLoading
			});
		}
	},
	...
	getPixelRadiusFromUnits: function(val) {
		var r = val / this.map.getResolution();
